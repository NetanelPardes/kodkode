...
namespace SmartLockerApi.Repositories
{
    public class LockerRepository:ILockerRepository
    {
        ...
        public Locker? Update (int id , Locker updateLocker )
        {
            var existing = _lockers.FirstOrDefault(l => l.Id == id);
            if (existing == null)
            {
                return null;
            }
            existing.LockerNumber = updateLocker.LockerNumber;
            existing.Status = updateLocker.Status;
            existing.AssignedTo = updateLocker.AssignedTo;
            existing.EquipmentType = updateLocker.EquipmentType;
            existing.AssignedAt = updateLocker.AssignedAt;
        ...
        {
            var locker = _lockers.FirstOrDefault(l => l.Id == id);
